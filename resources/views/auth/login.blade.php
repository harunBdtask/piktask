...
@section('content')
    ...
    <div class="container margin-bottom-40">
        ...
        <div class="row">
            ...
            <div class="@if ($settings->registration_active == 1) col-md-6 @else col-md-12 @endif">
                ...
                <div class="login-form">
                    ...
                    <form action="{{ url('login') }}" method="post" name="form" id="signup_form">
                        ...
                        <div class="form-group has-feedback">
                            ...
                            <input type="text" class="form-control login-field custom-rounded" value="{{ old('email') }}"
                                name="email" id="email" placeholder="{{ trans('auth.username_or_email') }}"
                                title="{{ trans('auth.username_or_email') }}" autocomplete="off">
                            <span class="glyphicon glyphicon-user form-control-feedback"></span>
                        </div>


                        <div class="form-group has-feedback">
                            <input type="password" class="form-control login-field custom-rounded" name="password"
                                id="password" placeholder="{{ trans('auth.password') }}"
                                title="{{ trans('auth.password') }}" autocomplete="off">
                            ...
                            <div class="col-xs-5">
                                <label class="btn-block">
                                    <a href="{{ url('password/reset') }}"
                                        class="label-terms recover">{{ trans('auth.forgot_password') }}</a>
                                </label>
                            </div>
                        </div><!-- row -->

                        <button type="submit" id="buttonSubmit"
                            class="btn btn-block btn-lg btn-main custom-rounded">{{ trans('auth.sign_in') }}</button>

                        @if ($settings->facebook_login == 'on' || $settings->twitter_login == 'on')
                            <span class="login-link auth-social"
                                id="twitter-btn-text">{{ trans('auth.or_sign_in_with') }}</span>
                        @endif

                        @if ($settings->facebook_login == 'on')
                            <div class="facebook-login auth-social" id="twitter-btn">
                                <a href="{{ url('oauth/facebook') }}"
                                    class="btn btn-block btn-lg facebook custom-rounded"><i class="fa fa-facebook"></i>
                                    Facebook</a>
                            </div>
                        @endif
                        ...
                        @if ($settings->twitter_login == 'on')
                            <div class="facebook-login auth-social" id="twitter-btn">
                                <a href="{{ url('oauth/twitter') }}"
                                    class="btn btn-block btn-lg twitter custom-rounded"><i class="fa fa-twitter"></i>
                                    Twitter</a>
                            </div>
                        @endif
                    </form>
            ...
            @if ($settings->registration_active == 1)
                ...
                <div class="col-md-6">
                    ...
                    <div class="btn-block text-center">
                        <a href="{{ url('register') }}"
                            class="btn btn-lg btn-success custom-rounded">{{ trans('auth.sign_up') }}</a>
                    </div>
